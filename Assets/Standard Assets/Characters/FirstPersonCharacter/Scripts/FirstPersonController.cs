...
namespace UnityStandardAssets.Characters.FirstPerson {
    ...
    public class FirstPersonController : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            {
                m_Cylinder.SetActive(!m_Cylinder.activeSelf);
            }
        ...
        private void FixedUpdate()
        {
            ...
            if (m_CharacterController.isGrounded)
            {
                ...
                if (m_Jump)
                {
                    // disallow the player from jumping
        ...
        private void OnControllerColliderHit(ControllerColliderHit hit)
        {
            ...
            if (hit.gameObject.name == "Machine")
            {
                m_Card.SetActive(true);
            }
            if (hit.gameObject.name.StartsWith("Door"))
            {
                if (m_Card.activeSelf)
                {
                    m_AudioSource.clip = m_TapSound;
                    m_AudioSource.PlayOneShot(m_AudioSource.clip);
                    m_Card.SetActive(false);
                    hit.gameObject.SetActive(false);
                }
            }
            if (hit.gameObject.name == "Train01")
            {
                if (m_TextLabel.text.Trim() == "")
                {
                    m_TextLabel.text = "Time Taken: " + Time.timeSinceLevelLoad + " seconds, Press R to Restart";
                }
            }
